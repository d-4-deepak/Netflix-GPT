 ...
 const handleGptSearchClick = async ()=>{
  ...
  return (
    ...
    <form className='bg-black grid grid-cols-12 w-full md:w-3/4 lg:w-1/2' onSubmit={(e)=>e.preventDefault()}>
        ...
        <button disabled={!useSearchText} onClick={handleGptSearchClick} className='flex items-center justify-center disabled:bg-gray-200 disabled:text-black m-4 px-4 py-2 sm:px-4 sm:py-2 bg-red-700 rounded-lg text-white col-span-3 cursor-pointer hover:bg-red-800 sm:text-[16px] text-[6px] text-[10px]'>{lang[selectedLang]?.search}</button>
    </form>
</div> 
  )
}
